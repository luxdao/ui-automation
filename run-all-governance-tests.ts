...
async function runAll() {
  ...
  let mdTotalRunTimeStr = '';
  if (mdTotalDuration >= 60) {
    mdTotalRunTimeStr = (mdTotalDuration / 60).toFixed(1) + ' min';
  } else {
    mdTotalRunTimeStr = mdTotalDuration.toFixed(2) + 's';
  }
  const mdSummaryStats = `**Summary:** ${mdTotalPassed}/${mdTotalCount} tests passed  |  Total runtime: ${mdTotalRunTimeStr}\n\n`;
