...
export async function generateCombinedSummary(
  ...
  // Pre-calculate test counts for summary
  ...
  // Build markdown with correct order from the start
  let combinedMd = `# 🧪 UI Automation Test Results\n\n**Start Timestamp:** ${firstTimestamp}  \n`;
  combinedMd += mdSummaryStats; // Add summary stats right after timestamp
  ...
  // Generate table rows (reuse the same sorted test names)
  for (const testName of sortedTestNames) {
    const row = [testName];
