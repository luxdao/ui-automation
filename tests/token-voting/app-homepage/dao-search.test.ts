...
BaseSeleniumTest.run(async (test) => {
  ...
  const searchInput = await test.waitForElement(By.css('[data-testid="search-input"]'), 10000);
  await searchInput.click();
  await searchInput.sendKeys('0xB4b01b4Dc5f8d11feD90D760a237BF4D74C3423d', Key.RETURN);
  // Wait for the expected text to appear
  await test.waitForElement(By.xpath("//*[contains(text(), 'ERC-20 token DAO')]") , 10000);
